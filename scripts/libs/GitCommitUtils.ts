...
function parseCoAuthors(message: string): GitHubCoAuthor[] {
  return [...message.matchAll(/^Co-authored-by:\s+(.+?)<([^>]+)>$/gim)].map(
    (match) => ({
...
function resolveDisplayNameToLogin(displayName: string): string | null {
  ...
  const candidates = [
    trimmedDisplayName,
    trimmedDisplayName.replaceAll(/\s+/g, ""),
  ];
...
function resolveCoAuthorToLogin(
  coAuthor: GitHubCoAuthor,
  allowedLogins?: Set<string>,
): string | null {
