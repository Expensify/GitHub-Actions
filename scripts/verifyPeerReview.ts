...
function getCommitAuthors(
  ...
  for (const commit of commits) {
    ...
    for (const coAuthor of GitCommitUtils.parseCoAuthors(
      commit.commit.message,
    )) {
      const login = GitCommitUtils.resolveCoAuthorToLogin(
        coAuthor,
        employeeLogins,
      );
      if (login) {
  ...
  return {
    ...
    unresolvedExpensifyCoAuthors: CollectionUtils.uniqueSorted([
...
function evaluatePeerReview(input: PeerReviewInput): PeerReviewResult {
  ...
  if (authors.every((author) => GitHubUtils.isBotUser(author))) {
    return {
      status: "fail",
      error: new Error(
        `Unable to verify independent peer review because ${prSlug} has no human commit authors or co-authors.`,
      ),
    };
  }
...
async function main(): Promise<void> {
  ...
  const employeeLogins =
    requiredApprovingReviewCount > 0
      ? await GitHubUtils.getEmployeeLogins()
      : new Set<string>();
  const { authors, unresolvedExpensifyCoAuthors } = getCommitAuthors(
    commits,
    employeeLogins,
  );
