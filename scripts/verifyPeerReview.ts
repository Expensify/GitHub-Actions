...
async function main(): Promise<void> {
    ...
    const {authors, unresolvedExpensifyCoAuthors} = CoAuthors.getCommitAuthors(commits);

    const employeeLogins = approvers.length > 0 && requiredApprovingReviewCount > 0 ? await PeerReviewGitHubApi.getEmployeeLogins() : new Set<string>();
