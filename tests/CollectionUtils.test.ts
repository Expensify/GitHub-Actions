...
describe('asyncFilter', () => {
    it('keeps items whose predicate resolves true', async () => {
        const result = await CollectionUtils.asyncFilter([1, 2, 3, 4], async (value) => value % 2 === 0);

        assert.deepEqual(result, [2, 4]);
    });

    it('preserves input order', async () => {
        const result = await CollectionUtils.asyncFilter(['c', 'a', 'b'], async () => true);

        assert.deepEqual(result, ['c', 'a', 'b']);
    });

    it('returns an empty array when nothing matches', async () => {
        const result = await CollectionUtils.asyncFilter([1, 2, 3], async () => false);
