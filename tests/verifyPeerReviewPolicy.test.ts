...
import VerifyPeerReview, {
  type PeerReviewInput,
} from "../scripts/verifyPeerReview";
...
describe("evaluatePeerReview", () => {
  ...
  it("fails when all authors are bots", () => {
    ...
    if (result.status === "fail") {
      assert.match(
        result.error.message,
        /no human commit authors or co-authors/,
      );
    }
  });
  ...
  it("fails on unresolved expensify co-author emails", () => {
    ...
    if (result.status === "fail") {
      assert.match(
        result.error.message,
        /Unable to resolve Expensify co-author emails/,
      );
    }
  });
});
...
describe("getFailureTitle", () => {
  it("maps failure titles for known messages", () => {
    assert.equal(
      VerifyPeerReview.getFailureTitle(
        "Unable to resolve canonical commit author: missing GitHub author login and commit author name.",
      ),
