...
describe('resolveCoAuthorLogin', () => {
    it('parses standard noreply addresses', () => {
        assert.equal(CoAuthors.resolveCoAuthorLogin('AndrewGable@users.noreply.github.com'), 'AndrewGable');
    });

    it('parses numeric noreply prefixes', () => {
        assert.equal(CoAuthors.resolveCoAuthorLogin('2838819+AndrewGable@users.noreply.github.com'), 'AndrewGable');
    });
});
...
describe('coAuthorEmails', () => {
    it('extracts multiple co-author emails', () => {
        ...
        assert.deepEqual(CoAuthors.coAuthorEmails(message), ['AndrewGable@users.noreply.github.com', 'MonilBhavsar@users.noreply.github.com']);
    });
});
...
describe('getCommitAuthors', () => {
    ...
    it('ignores co-authors when canonical author is human', () => {
        const result = CoAuthors.getCommitAuthors([makeCommit('rafecolton', undefined, 'Change\n\nCo-authored-by: Andrew Gable <AndrewGable@users.noreply.github.com>')]);

        assert.deepEqual(result.authors, ['rafecolton']);
    });

    it('falls back to commit author name when github login is missing', () => {
        const result = CoAuthors.getCommitAuthors([makeCommit(undefined, 'AndrewGable', 'Change')]);
