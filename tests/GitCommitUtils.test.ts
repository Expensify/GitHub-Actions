...
describe("parseCoAuthors", () => {
  it("extracts display names and emails", () => {
    ...
    assert.deepEqual(GitCommitUtils.parseCoAuthors(message), [
      { displayName: "Andrew Gable", email: "andrew@expensify.com" },
    ]);
  });
});
...
describe("resolveDisplayNameToLogin", () => {
  it("parses github usernames from display names", () => {
    assert.equal(
      GitCommitUtils.resolveDisplayNameToLogin("AndrewGable"),
      "AndrewGable",
    );
  });
  ...
  it("removes spaces from display names", () => {
    assert.equal(
      GitCommitUtils.resolveDisplayNameToLogin("Andrew Gable"),
      "AndrewGable",
    );
  });
  ...
  it("returns null when display name cannot map to a github username", () => {
    assert.equal(
      GitCommitUtils.resolveDisplayNameToLogin("John O'Brien"),
      null,
    );
  });
});
...
describe("resolveCoAuthorToLogin", () => {
  ...
  it("rejects display names that do not match allowed logins", () => {
    assert.equal(
      GitCommitUtils.resolveCoAuthorToLogin(
        { displayName: "John Smith", email: "andrew@expensify.com" },
        new Set(["AndrewGable"]),
      ),
      null,
    );
...
describe("getCanonicalAuthorLogin", () => {
  it("returns github author login when present", () => {
    assert.equal(
      GitCommitUtils.getCanonicalAuthorLogin(
        makeCommit("AndrewGable", undefined, "Change"),
      ),
      "AndrewGable",
    );
  });
  ...
  it("throws when canonical author cannot be resolved", () => {
    assert.throws(
      () =>
        GitCommitUtils.getCanonicalAuthorLogin(
          makeCommit(undefined, undefined, "Change"),
        ),
      /Unable to resolve canonical commit author/,
    );
  });
});
